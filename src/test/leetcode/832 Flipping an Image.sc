...
To flip an image horizontally means
that each row of the image is reversed.
...
def flipAndInvertImage(A: Array[Array[Int]]): Array[Array[Int]] = {
  A.map(_.reverse.map(x=>1-x))
}


val x= flipAndInvertImage(Array(Array(1,0),Array(1,0)))
x.toList.map(_.toList)
